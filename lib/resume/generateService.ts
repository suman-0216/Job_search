...
export type GenerateResumeOutput = {
  ok: true
  markdown: string
  provider: string
  model: string
...
export const generateResumeForUser = async (input: GenerateResumeInput): Promise<GenerateResumeOutput> => {
  const userId = String(input.userId || '').trim()
  if (!userId) throw new ResumeGenerationError(401, 'Unauthorized')

  const jobDescription = String(input.jobDescription || '').slice(0, 80_000)
  const templateMarkdown = String(input.templateMarkdown || '').slice(0, 120_000)
  const promptA = String(input.promptA || DEFAULT_ATS_PROMPT).slice(0, 12_000)
  const resumeOverride = String(input.resumeText || '').slice(0, 120_000)

  if (!jobDescription.trim()) throw new ResumeGenerationError(400, 'Job description is required.')
  ...
  const { data: settings, error: settingsError } = await supabase
    ...
    .eq('user_id', userId)
    .maybeSingle()
  if (settingsError) throw new ResumeGenerationError(500, settingsError.message)
  if (!settings) throw new ResumeGenerationError(400, 'User settings not found.')

  const provider = toStringValue(settings.llm_provider).toLowerCase()
  const model = toStringValue(settings.llm_model) || DEFAULT_MODEL_BY_PROVIDER[provider] || ''
  const llmValidationError = validateLlmProviderModelAndKey({
    provider,
    model,
    apiKey: settings.llm_api_key,
  })
  if (llmValidationError) throw new ResumeGenerationError(400, llmValidationError)
  ...
  const { data: profileData, error: profileError } = await supabase
    ...
    .eq('user_id', userId)
    .maybeSingle()
  if (profileError) throw new ResumeGenerationError(500, profileError.message)
  ...
  if (!resumeText.trim()) throw new ResumeGenerationError(400, 'Resume text is required. Upload resume or paste text first.')
  ...
  try {
    ...
    if (!markdown) throw new ResumeGenerationError(502, 'LLM returned an empty response.')
    ...
    throw new ResumeGenerationError(500, error instanceof Error ? error.message : 'Failed to generate tailored resume')
  }
}
